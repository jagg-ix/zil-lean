...
Build the knowledge module first (this produces the `.olean` the consumer
imports), then run the consumer:
...
## 6. Project verification arc

```bash
lake env lean examples/lean/06_ProjectVerificationArc.lean
```

Introduces a synthetic project-only scenario:

- implementation, gate, requirement, and release nodes;
- derived certification instead of asserted status;
- release-availability derivation;
- a multi-answer query over the derived relations.

The example contains no external repository names, private branch identifiers,
commit IDs, personal paths, or domain research results.
