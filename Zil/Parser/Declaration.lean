...
private def parseNamedTerm (token : String) : Except String Term :=
  let clean := if token.startsWith ":" then (token.drop 1).toString else token
...
private partial def parseValue (text : String) : Except String DeclValue := do
  ...
  if token.isEmpty then throw "empty declaration value"
    ...
    pure (.scalar (.text ((token.drop 1).dropRight 1).toString))
    ...
    let payload := ((token.drop 2).dropRight 1).toString
    ...
    let payload := ((token.drop 1).dropRight 1).toString
    let entries ←
