...
private def parseAttributes (line : SourceLine) (text : String) : Except ParseError (Array Attribute) := do
  ...
  for rawEntry in text.splitOn "," do
    let (keyText, valueText) ← match rawEntry.splitOn "=" with
      | key :: rest@(_ :: _) => pure (key.trim, (String.intercalate "=" rest).trim)
      | _ => failAt line s!"invalid attribute entry: {rawEntry.trim}"
    if keyText.isEmpty || valueText.isEmpty then
      throw { line := line.number, message := "attribute key and value must be nonempty", sourceLine := line.text }
...
private def splitAtomAndAttributes
  ...
  | [atom, attrs] =>
      unless attrs.trim.endsWith "]" do
        throw { line := line.number, message := "attribute list must end with ']'", sourceLine := line.text }
...
private def parseRelationAtom (line : SourceLine) (text : String) : Except ParseError RelExpr := do
  ...
  if objectText.contains '#' then
    throw { line := line.number, message := "userset selectors are accepted on stored tuples, not rule/query atoms", sourceLine := line.text }
...
private def parseBody (line : SourceLine) (text : String) : Except ParseError ParsedBody := do
  ...
  for piece in pieces do
    ...
    if literal.isEmpty then
      throw { line := line.number, message := "empty atom in relation conjunction", sourceLine := line.text }
    if literal.startsWith "NOT " then
      let atomText := literal.drop 4 |>.trim.toString
      if atomText.isEmpty then
        throw { line := line.number, message := "NOT requires a relation atom", sourceLine := line.text }
...
private def parsePositiveConjunction
  ...
  unless body.negative.isEmpty do
    throw { line := line.number, message := "NOT is allowed in rule/query bodies only", sourceLine := line.text }
  pure body.positive
...
private def parseHeaderName
    (line : SourceLine) (pre : String) : Except ParseError Name := do
  let text := line.text.trim
  unless text.startsWith pre && text.endsWith ":" do
    throw { line := line.number, message := s!"expected {pre}<name>:", sourceLine := line.text }
  let nameText := (text.drop pre.length).dropRight 1 |>.trim.toString
...
private def parseRule
  ...
  unless ifText.startsWith "IF " do
    throw { line := ifLine.number, message := "rule body must begin with IF", sourceLine := ifLine.text }
  let thenText := thenLine.text.trim
  unless thenText.startsWith "THEN " && thenText.endsWith "." do
    throw { line := thenLine.number, message := "rule head must begin with THEN and end with '.'", sourceLine := thenLine.text }
  let body ← parseBody ifLine (ifText.drop 3).toString
  if body.positive.isEmpty then
    throw { line := ifLine.number, message := "rule requires at least one positive premise", sourceLine := ifLine.text }
  let heads ← parsePositiveConjunction thenLine ((thenText.drop 5).dropRight 1).toString
  ...
  unless negativeVariables.all positiveVariables.contains do
    throw { line := ifLine.number, message := "negative premise contains a variable not bound by a positive premise", sourceLine := ifLine.text }
  unless headVariables.all positiveVariables.contains do
    throw { line := thenLine.number, message := "rule head contains a variable not bound by the positive body", sourceLine := thenLine.text }
...
private def selectedVariable (line : SourceLine) (token : String) : Except ParseError Name := do
  ...
  unless value.startsWith "?" do
    throw { line := line.number, message := s!"FIND entry must be a variable: {value}", sourceLine := line.text }
  (nameFromToken (value.drop 1).toString).mapError fun message =>
    { line := line.number, message, sourceLine := line.text }
...
private def parseQuery (header findLine : SourceLine) : Except ParseError Query := do
  ...
  unless text.startsWith "FIND " && text.endsWith "." do
    throw { line := findLine.number, message := "query must begin with FIND and end with '.'", sourceLine := findLine.text }
  let payload := (text.drop 5).dropRight 1 |>.trim.toString
  let (selectText, whereText) ← match payload.splitOn " WHERE " with
    | [select, whereBody] => pure (select, whereBody)
  ...
  if select.isEmpty then
    throw { line := findLine.number, message := "query FIND list is empty", sourceLine := findLine.text }
  let body ← parseBody findLine whereText
  if body.positive.isEmpty then
    throw { line := findLine.number, message := "query requires at least one positive premise", sourceLine := findLine.text }
  let variables := relationsVariables body.positive
  let negativeVariables := relationsVariables body.negative
  unless negativeVariables.all variables.contains do
    throw { line := findLine.number, message := "negative query premise contains an unbound variable", sourceLine := findLine.text }
  unless select.all variables.contains do
    throw { line := findLine.number, message := "query selects a variable not bound by WHERE", sourceLine := findLine.text }
  let query : Query := {
...
private partial def parseLines
  ...
  if text.startsWith "MODULE " then
    ...
    let value := (text.drop 7).dropRight 1 |>.trim.toString
