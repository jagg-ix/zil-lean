...
def checkReferences
  ...
  | none => issues
  | some attr =>
      references pre attr.value |>.foldl (init := issues) fun out reference =>
...
def issues (declarations : Array Declaration) : Array DeclarationIssue :=
  let localIssues := declarations.foldl (init := #[]) fun out declaration =>
    out ++ declaration.issues
  let duplicates := declarations.foldl (init := localIssues) fun out declaration =>
