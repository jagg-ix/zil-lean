...
def encodeRule (rule : Zil.Rule) : String :=
  ...
  String.intercalate "\n" (#[header] ++ premises ++ negatives ++ #[conclusion]).toList
...
def decodeRule (text : String) : Except String Zil.Rule := do
  ...
  let conclusionValue ← conclusion.toExcept "canonical rule has no conclusion"
  pure { name := ruleName, variables, premises, negativePremises,
         conclusion := conclusionValue, trust }
