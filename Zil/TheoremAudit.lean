...
private def theoremContract
  ...
  let lemmas := objectsFor program thm `zil.requiresLemma
  let guarantees := objectsFor program thm `zil.ensures
  let evidence := proofEvidence program thm
  let criticality := theoremCriticality program thm
  let unconditional := trueFact program thm `zil.unconditional
...
def audit (program : Zil.Program) : Except String Report := do
  unless program.valid do throw "thm audit requires a structurally valid program"
  let moduleName ← match program.moduleName with
    | some value => pure value
    | none => throw "thm audit requires MODULE"
  ...
  let theoremFailures := (theorems.filter fun thm => !thm.ok).size
...
def render (report : Report) : String :=
  let theoremRows := report.theorems.toList.map fun thm =>
    String.intercalate "\t" [
      "theorem", thm.thm.toString,
      if thm.ok then "pass" else "fail",
      thm.criticality.token,
      if thm.nonvacuous then "nonvacuous" else "vacuous",
      if thm.unconditional then "unconditional" else "conditional",
      namesText thm.assumptions,
      namesText thm.lemmas,
      namesText thm.guarantees,
      namesText thm.proofEvidence,
      stringsText thm.issues
    ]
  ...
  String.intercalate "\n" <|
    ["ZIL-THEOREM-AUDIT\t1",
     ...
     "thm-failures\t" ++ toString report.theoremFailures,
