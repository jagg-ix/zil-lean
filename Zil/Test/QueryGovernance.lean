...
-- Dangling query_pack references are rejected fail-closed at parse time, so
-- governance never sees the program; assert the parser-level rejection.
#guard match Zil.Parser.DeclarationProgram.parseText missingPackSource with
  | .ok _ => false
  | .error error =>
      error.message == "attribute query_pack references missing declaration query_pack.missing"
