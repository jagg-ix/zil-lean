...
#guard match parsed with
  | .ok program =>
      match audit program with
      | .ok report =>
          match report.theorems.find? (fun thm => thm.thm == `theorem.valid) with
          | some thm =>
              thm.ok && thm.nonvacuous && !thm.unconditional &&
              thm.assumptions == #[`assumption.finite] &&
              thm.lemmas == #[`lemma.closure] &&
              thm.guarantees == #[`guarantee.sound] &&
              thm.proofEvidence == #[`proof.valid]
          | none => false
      | .error _ => false
  | .error _ => false
...
#guard match parsed with
  | .ok program =>
      match audit program with
      | .ok report =>
          match report.theorems.find? (fun thm => thm.thm == `theorem.unconditional) with
          | some thm => thm.ok && thm.unconditional && thm.nonvacuous
          | none => false
      | .error _ => false
  | .error _ => false
...
#guard match parsed with
  | .ok program =>
      match audit program with
      | .ok report =>
          let vacuous := report.theorems.find? (fun thm => thm.thm == `theorem.vacuous)
          let missing := report.theorems.find? (fun thm => thm.thm == `theorem.missingLemma)
          let evidence := report.theorems.find? (fun thm => thm.thm == `theorem.noEvidence)
