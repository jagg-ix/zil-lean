...
(defn validate-inventory!
  ...
  (let [capabilities (vec (:capabilities inventory))
        ...
        operations (keepv :operation capabilities)
    ...
    (doseq [{:keys [id authority assurance operation] :as capability} capabilities]
      (when-not (keyword? id)
        (throw (ex-info "capability id must be a keyword"
                        {:kind :configuration-error :capability capability})))
    ...
    (doseq [[operation capability] worker-protocol/operation-capabilities]
