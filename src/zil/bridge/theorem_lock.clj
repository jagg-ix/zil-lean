...
(defn- value
  "Sorted maps compare keys, so probing a keyword-keyed map with a string
  throws; scan entries instead of relying on the comparator."
  [m key]
  (let [kw (keyword key)]
    (reduce-kv (fn [_ k v]
                 (if (or (= k key) (= k kw)) (reduced v) nil))
               nil m)))

(defn- without-key [m key]
  (let [kw (keyword key)]
    (into (empty m) (remove (fn [[k _]] (or (= k key) (= k kw))) m))))
