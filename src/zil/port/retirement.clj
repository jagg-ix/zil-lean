...
(defn- embedded-fully-verified? [embedded]
  ;; Zero blocks is vacuous evidence, not verification.
  (and (:verify-generated embedded)
       (pos? (:block-count embedded 0))
...
(defn evaluate
  ...
  (let [repository-root (canonical-file (:repository-root policy "."))
        ...
        non-blocking-prefixes (:non-blocking-prefixes policy ["test/" "examples/"])
...
(def default-policy-data
  {:repository-root "."
   :scan-roots ["bin" "src" "test" "examples" "deps.edn" "build.clj"]
   :non-blocking-prefixes ["test/" "examples/"]
   :components {}})
