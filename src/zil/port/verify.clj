...
(defn- absolute-path ^Path [value]
  (let [path (.normalize (.toAbsolutePath (Paths/get (str value) (make-array String 0))))]
    ;; Resolve symlinks (macOS /var -> /private/var) so stored paths match
    ;; canonical paths used by consumers; fall back for not-yet-created paths.
    (try
      (.toRealPath path (make-array java.nio.file.LinkOption 0))
      (catch java.io.IOException _ path))))
...
(defn verify-manifest!
  ...
  (let [manifest-data (read-manifest! manifest)
        ...
        aggregate-path (let [p (.normalize (.toAbsolutePath (java.nio.file.Paths/get (str aggregate) (make-array String 0))))
                             parent (.getParent p)]
                         (path-string (if parent
                                        (.resolve (absolute-path (.toString parent)) (.getFileName p))
                                        p)))
